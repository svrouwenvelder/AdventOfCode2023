...
def process_almanac(path_to_file: str) -> tuple[list[int], Almanac]:
    ...
    return seeds, Almanac(seed2soil, soil2fertilizer, fertilizer2water, water2light, light2temperature, temperature2humidity, humidity2location)

def get_lowest_location_number(seeds: list[int], almanac: Almanac) -> int:
    return min(almanac.get_location(seed) for seed in seeds)

def get_lowest_location_number_with_ranges(seed_start: int, seed_stop: int, almanac: Almanac) -> int:
    return min(almanac.get_location(seed) for seed in range(seed_start, seed_start+seed_stop))
...
if __name__ == "__main__":
    ...
    print("PART2")
    print(min(get_lowest_location_number_with_ranges(seeds[i], seeds[i+1], almanac) for i in range(0, len(seeds), 2)))
